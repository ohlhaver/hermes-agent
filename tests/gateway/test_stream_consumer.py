...
class TestInterimCommentaryMessages:
    ...
    @pytest.mark.asyncio
    async def test_rejected_native_commentary_does_not_block_or_duplicate_final(self):
        from gateway.config import Platform, PlatformConfig
        from gateway.platforms.base import BasePlatformAdapter, SendResult

        class RejectingActivityAdapter(BasePlatformAdapter):
            def __init__(self):
                super().__init__(PlatformConfig(enabled=True), Platform.TELEGRAM)
                self.commentary = []
                self.sent = []
        ...
        adapter = RejectingActivityAdapter()
        consumer = GatewayStreamConsumer(
            adapter,
            "chat_123",
            StreamConsumerConfig(edit_interval=0.01, buffer_threshold=5),
        ...
        consumer.on_commentary("Unsafe or stale activity.")
        consumer.on_delta("Clean final answer.")
        consumer.finish()
        await consumer.run()

        assert adapter.commentary == ["Unsafe or stale activity."]
        assert adapter.sent == ["Clean final answer."]
